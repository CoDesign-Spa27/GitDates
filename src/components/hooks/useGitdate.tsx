...
export const useGitDate = () => {
    ...
    const createGitDateProfile = useMutation({
        mutationFn: (githubData: UserData) => createGitDateProfileMutation(githubData),
        onMutate: async (newData: UserData) => {

            await queryClient.cancelQueries({ queryKey: ['gitdate-profile-get', email] })

            const previousProfile = queryClient.getQueryData(['gitdate-profile-get', email])
        ...
        onSettled: () => {
            queryClient.invalidateQueries({ queryKey: ['gitdate-profile-get', email] });
        },
    })
